...
| `VLLM_OMNI_AR_DIFFUSION_ALLOW_REFERENCE_ATTN` | `1`, `true`, `yes` or `on` enables; default `0` (off) | AR-Diffusion paged self-attention; read when the attention backend configuration is constructed (KV cache initialization, and LingBot self-attention cache allocation) | Environment-only setting. Values are stripped and case-normalized. **Diagnostic only.** It is consulted only when setup finds no paged FlashAttention varlen entry point bound for the device: that raises during initialization by default, and enabling this substitutes the dense Python reference instead, which is far slower. It *allows* that fallback rather than forcing it — an available entry point still selects the native kernel. It does not intercept runtime kernel errors: an unsupported kernel geometry propagates. Changing it mid-process does not alter a configuration that already exists. Do not set it in a deployment. | Diagnostic |
