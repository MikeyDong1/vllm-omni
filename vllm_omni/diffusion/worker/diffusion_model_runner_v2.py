...
"""Stepwise diffusion runner using state-based pipeline atoms (PR #4948).

Runner-side execution for the #4948 :class:`DiffusionV2Atoms` step contract:
the runner owns state cache, batching, the step loop, scatter/gather, metrics,
and local stage execution; the pipeline owns the model math and the state atoms
(``init_state`` / ``check_inputs`` / ``encode`` / ``prepare`` / ``build_step_batch``
/ ``denoise_step`` / ``step_scheduler`` / ``decode`` / ``postprocess``).

Adapted from PR #4948 to the current base-runner surface (post-#5216 attribute
names: ``_target_device`` / ``_supports_step_mode`` / ``_initialize_generator``).
Pipelines that implement only the legacy :class:`SupportsStepExecution` contract
(helios / hunyuan_image3 / qwen_image / diffusers_adapter) continue to use the
base :class:`DiffusionModelRunner`'s ``execute_stepwise`` path; this v2 runner is
used only by pipelines migrated to the ``DiffusionV2Atoms`` atoms.
"""
...
class DiffusionModelRunnerV2(DiffusionModelRunner):
    """Runner-side execution for #4948 DiffusionV2Atoms.
